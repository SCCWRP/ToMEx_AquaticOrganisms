...
species <- data %>%
  ...
  group_by(db, species_f, env_f, exposure.route) %>%
  summarise()
...
data <- data %>% 
  mutate(poly_f = case_when(
    ...
    poly_f == "High Density Polyethylene" ~ "PE",
    poly_f == "Low Density Polyethylene" ~ "PE",
    poly_f == "Medium Density Polyethylene" ~ "PE",
...
poly <- full_join(poly1,poly2) %>% 
  ungroup() %>% 
  filter(poly_f != "MIX") %>% 
  add_row(db = "ToMEx 1.0", poly_f = "PVA", poly_obs = 0)%>% 
  add_row(db = "ToMEx 1.0", poly_f = "S/PBAT/PLA", poly_obs = 0)%>% 
  add_row(db = "ToMEx 1.0", poly_f = "TW", poly_obs = 0)%>% 
  add_row(db = "ToMEx 1.0", poly_f = "SP", poly_obs = 0)%>% 
  add_row(db = "ToMEx 1.0", poly_f = "PVC-VA", poly_obs = 0)%>% 
  add_row(db = "ToMEx 1.0", poly_f = "PS-CO-ACN", poly_obs = 0)%>% 
  add_row(db = "ToMEx 1.0", poly_f = "POM", poly_obs = 0)%>% 
  add_row(db = "ToMEx 1.0", poly_f = "PTFE", poly_obs = 0)%>%
  mutate(db=as.factor(db))
...
ggsave("Figures/POLY_AQ.png", plot = poly_plot, width = 10, height = 14, units = "in")
...
ggsave("Figures/SIZE_SHAPE_AQ.png", plot = size_shape, width = 10, height = 14, units = "in")
...
ggsave("Figures/ENDPT_AQ.png", plot = endpt_plot, width = 10, height = 7, units = "in")
...
ggsave("Figures/CRITERIA_AQ.png", plot = red_crit_plot, width = 8, height = 6, units = "in")
