...
tomex2.0_aoc_setup$density.g.cm3[tomex2.0_aoc_setup$poly_f == "Polyamidoamine"] = 1.22
  ...
  bodysize_summary <- aoc_setup %>%
    ...
    mutate(species_f = as.character(species_f))
  ...
  bodysize_summary_complete <- bind_rows(bodysize_summary, bodysize_addons) %>%
    drop_na(max.size.ingest.um) %>% 
    distinct(species_f, .keep_all = T)
  
  #Join summary to tidy ToMEx 2.0 data frame
  tomex2.0_aoc_setup_joined <- left_join(tomex2.0_aoc_setup %>% mutate(species_f = as.character(species_f)), 
                                         bodysize_summary_complete, by = c("species_f")) %>% 
    mutate(body.length.cm = case_when(species_f == "Apostichopus japonicus" ~ 10, T ~ body.length.cm)) #I cannot figure out why this row is effectively unwriteable?!
 
  ##################### PREPARATION FUNCTION ########################
  tomex2.0_aoc_setup_prepared <- preparation_fxn(df = tomex2.0_aoc_setup_joined,
    ...
    ### resume cleanup
  tomex2.0_aoc_setup <- tomex2.0_aoc_setup_prepared %>% 
...
names <- tomex2.0_aoc_setup %>%
  select(-c(nominal.dose...mass, nominal.dose...mass.units, nominal.dose...particles, nominal.dose...particles.units, 
            ...
            dose.mg.kg.sed.measured, dose.mg.kg.sed.nominal, dose.particles.kg.sed.nominal, beta_log10_body_length, 
            body_length_intercept, R.ave.water.marine, R.ave.water.freshwater, R.ave.sediment.marine, R.ave.sediment.freshwater, 
            p.ave.marine, alpha.marine, a.sa.marine, a.v.marine, a.m.marine, a.ssa.marine, p.ave.freshwater, alpha.freshwater, 
            a.sa.freshwater, a.v.freshwater, a.m.freshwater, a.ssa.freshwater
            )) %>% 
  colnames() 
...
cat(yellow("tomex2.0_aoc_z_final saved!"))
